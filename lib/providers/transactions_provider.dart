...
import '../repository/interfaces/recurring_transaction_methods.dart';
...
class AsyncTransactionsNotifier
  ...
  final IRecurringTransactionMethods _recurringTransactionMethods;

  AsyncTransactionsNotifier(
      {required IRecurringTransactionMethods recurringTransactionMethods})
      : _recurringTransactionMethods = recurringTransactionMethods;
  ...
  Future<RecurringTransaction?> addRecurringTransaction(
    ...
    state = await AsyncValue.guard(() async {
      insertedTransaction =
          await _recurringTransactionMethods.insert(transaction);
      return _getTransactions(update: true);
    });
  ...
  Future<void> updateRecurringTransaction(num amount, String label) async {
    ...
    state = const AsyncValue.loading();
    state = await AsyncValue.guard(() async {
      await _recurringTransactionMethods.updateItem(transaction);
      return _getTransactions(update: true);
    });
  }
  ...
  Future<void> deleteRecurringTransaction(int transactionId) async {
    state = const AsyncValue.loading();
    state = await AsyncValue.guard(() async {
      await _recurringTransactionMethods.deleteById(transactionId);
      return _getTransactions(update: true);
    });
  }
...
final transactionsProvider = AsyncNotifierProvider.autoDispose<
  ...
  return AsyncTransactionsNotifier(
    recurringTransactionMethods: RecurringTransactionMethods(),
  );
});
